...
const SmartSpeakers =()=> {
  ...
  const [speakersCategory, setSpeakersCategory] = useState([]);
  const [loading, setLoading] = useState(false);
  const [selectedOptions, setSelectedOptions] = useState({});


  const fetchSpeakersCategory = async () =>{
    setLoading(true)
    try {
      const res = await api.get("/products/smartspeakers");
      setSpeakersCategory(res.data.products);
 ...
 setSelectedOptions(initialOptions);
      ...
      toast.error("Failed to fetch speakers category");
      console.log(error);
  ...
  return (
    <>
      ...
      <div className="container my-5" style={{ marginTop: "70px" }}>
        <h1 className="text-center mb-4">Smart Speakers</h1>
